...
export class ApiClient {
  ...
  async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    const body = response.status === 204 ? undefined : contentType.includes('application/json') ? await response.json() : await response.text()
    if (!response.ok) throw new ApiError(response.status, (body as { detail?: unknown })?.detail ?? body)
    return body as T
  }
