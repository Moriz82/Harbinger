...
def backup(store, destination):
    ...
    with store.lock:
        ...
        if queue.exists():
            if queue.is_symlink() or not queue.is_dir() or any(queue.iterdir()):
                raise RuntimeError('Cannot back up a workspace with a non-empty parser queue')
...
def restore(source, destination):
    ...
    staging.mkdir(mode=0o700)
    try:
        for name in ('artifacts', 'conflicts', 'keys', 'staging', 'exports', 'parser-queue'):
            (staging / name).mkdir(mode=0o700)
    ...
    try:
        ...
        if args.command == 'add-user':
            ...
            try:
                ...
                from .ghostwriter import verify_adapter
                ...
                if old_token is None:
                    ...
                    os.replace(temporary, p)
                raise
            store.configure('ghostwriter', {**config, 'schema_hash': schema_hash})
