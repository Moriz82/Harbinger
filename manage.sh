...
prepare_dirs() {
  prepare_env
  local runtime_user runtime_uid runtime_gid directory owner permissions
  runtime_user=$(runtime_identity)
  runtime_uid=${runtime_user%%:*}
  runtime_gid=${runtime_user##*:}
  for directory in "$ROOT/state" "$ROOT/state/parser-queue"; do
    if [[ ! -e "$directory" ]]; then
      if [[ $(id -u) == 0 ]]; then
        install -d -o "$runtime_uid" -g "$runtime_gid" -m 700 "$directory"
      else
        [[ "$(id -u):$(id -g)" == "$runtime_user" ]] || fail 'Only root or the configured application user can create workspace directories.'
        install -d -m 700 "$directory"
      fi
    fi
    [[ -d "$directory" && ! -L "$directory" ]] || fail 'The workspace directory is not a private directory.'
...
require_services_stopped() {
  ...
  [[ -z "$running" ]] || fail 'Stop Harbinger before a host-side state change. Run ./manage.sh stop, then retry.'
}
mark_encrypted_path() {
...
case "${1:-help}" in
  plan)
    need_docker
    printf '%s\n' "$APP uses Docker Compose, port ${DEFAULT_PORT}, a private local state folder, and a networkless parser sidecar."
  ...
  init)
    ...
    user=${5:-captain}
    engagement_id=${6:-}
    peer_origin=${7:-http://harbinger:${DEFAULT_PORT}}
  ...
  status)
    ...
    ;;
  logs)
    need_docker; compose logs --tail 200 app parser
    ;;
  verify)
  ...
  backup)
    ...
    stage=$(mktemp -d "$parent/.harbinger-backup.XXXXXXXX")
    preserve_stage_on_failure() {
      local status=$? failed="$parent/harbinger-backup-failed-$$"
      trap - EXIT
...
PY
    ...
    stage=$(mktemp -d "$ROOT/.harbinger-restore.XXXXXXXX")
    preserve_stage_on_failure() {
      local status=$? failed="$ROOT/harbinger-restore-failed-$$"
      trap - EXIT
      if [[ $status -ne 0 && -d "${stage:-}" ]]; then
        mv -T -n -- "$stage" "$failed" || true
        [[ ! -d "$stage" ]] || failed=$stage
...
Usage: ./manage.sh COMMAND
  ...
  init [ENGAGEMENT MODE ORIGIN USER ENGAGEMENT-ID PEER-ORIGIN]
                               ...
                               Synthetic Docker peer origin defaults to http://harbinger:8710.
